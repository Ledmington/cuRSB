...
static rsb_err_t rsb__do_prec_build(struct rsb_mtx_t **mtxLpp, struct rsb_mtx_t **mtxUpp, const struct rsb_mtx_t *mtxAp)
{
	...
	if (rsb__allocate_coo_matrix_t(&ucoo) != &ucoo)
	{
		errval = RSB_ERR_ENOMEM;
		goto err;
	}
	...
	if (rsb__allocate_coo_matrix_t(&lcoo) != &lcoo)
	{
		errval = RSB_ERR_ENOMEM;
		goto err;
	}
...
#pragma omp parallel for schedule(static, gdc) reduction(| \
														 : errval) RSB_NTC
...
#pragma omp parallel for schedule(static, 1) reduction(| \
													   : errval) shared(mtxAp) RSB_NTC if (mtxAp->nnz > RSB_MIN_NNZ_FOR_PARALLEL_ADD_TO_DENSE)
...
rsb_err_t rsb__do_get_matrix_info(const struct rsb_mtx_t *mtxAp, enum rsb_mif_t miflags, void *info, size_t buflen)
{
	...
	*/
	...
	case RSB_MIF_MATRIX_ROWS__TO__RSB_COO_INDEX_T:
	{
		civ = (mtxAp->nr);
		if (buflen <= 0)
			*(rsb_coo_idx_t *)info = civ;
		else
			snprintf(cis, buflen, "%ld", (long int)civ);
	}
	break;
	...
	case RSB_MIF_MATRIX_NNZ__TO__RSB_NNZ_INDEX_T:
	{
		niv = (mtxAp->nnz);
		if (buflen <= 0)
			*(rsb_nnz_idx_t *)info = niv;
		else
			snprintf(cis, buflen, "%ld", (long int)niv);
	}
	break;
	...
	case RSB_MIF_MATRIX_FLAGS__TO__RSB_FLAGS_T:
	{
		fiv = (mtxAp->flags);
		if (buflen <= 0)
			*(rsb_flags_t *)info = fiv;
		else
			snprintf(cis, buflen, "%ld", (long int)fiv);
	}
	break;
	case RSB_MIF_MATRIX_TYPECODE__TO__RSB_TYPE_T:
	{
		...
		if (buflen <= 0)
			*(rsb_type_t *)info = tiv;
			...
			snprintf(cis, buflen, "%ld", (long int)tiv);
	}
	break;
	case RSB_MIF_MATRIX_INFO__TO__CHAR_P:
	{
		...
		if (buflen <= 0)
		{
			...
			snprintf(cis, buflen, RSB_PRINTF_MTX_SUMMARY_ARGS(mtxAp));
	...
	case RSB_MIF_LEAVES_COUNT__TO__RSB_BLK_INDEX_T:
	{
		...
		if (buflen <= 0)
			*(rsb_blk_idx_t *)info = biv;
		else
			snprintf(cis, buflen, "%d", biv);
	}
	break;
	default:
