...
struct rsb_mtx_t
{
	...
	rsb_nnz_idx_t nnz;		  /*! matrix (declared) nonzeros */
	rsb_coo_idx_t nr, nc;	  /*! matrix rows, columns */
...
#define RSB_PRINTF_MTX_SUMMARIZED_ARGS(PRE, MTXAP, POST)                                                                                                                                          \
	"%s(%zd x %zd)[%p]{%c} @ (%zd(%zd..%zd),%zd(%zd..%zd)) (%zd nnz, %.2lg nnz/r) flags 0x%x (coo:%d, csr:%d, hw:%d, ic:%d, fi:%d), storage: %zx, subm: %zd, symflags:'" RSB_PRINTF_FLAGS_FMT_STR \
	"'%s",                                                                                                                                                                                        \
		(PRE),                                                                                                                                                                                    \
